					...
					<div id="acc-characters-list" style="display: none;">
						...
						<?php $i=1; foreach($chars as $count) { ?>
							<div class='user-character-box'>
								...
								<div class='char-name'><a style='color:#<?php echo char_name_color($chars[$i]['class']); ?>' href='character.php?c=<?php echo $chars[$i]['name'];?>'><?php echo $chars[$i]['name'];?></a></div>
								<a class="userbox-link" href="character.php?c=Darksoke">VIEW CHARACTER EQUIPMENT</a>
							</div>
							<?php $i++; } ?>
					</div>
						...
						for ($i=1;$i<=count($vote_sites);$i++){
							...
							<div class='vote-box'>
								<div class='name-box'><?php echo $vote_sites[$i]['title'] ?></div>
									<?php if ($rem == 0){ ?>
										 <a id="vote-button" href="core/do_vote.php?siteid=<?php echo $vote_sites[$i]['id']; ?>&user=<?php echo $user_account->acc_id; ?>" target='_blank' onclick="location.reload(true)">
